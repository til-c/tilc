...
use crate::{Base, LiteralKind, Token, TokenKind};
...
impl<'a> Lexer<'a> {
  ...
  fn nth(&self, n: u32) -> char {
    ...
    return chars.nth(n as usize).unwrap_or_else(|| EOF_CHAR);
  }
  fn step(&mut self) -> char {
    return self.chars.next().unwrap_or_else(|| EOF_CHAR);
  }
  fn consume<F: FnMut(char) -> bool>(&mut self, mut until: F) {
